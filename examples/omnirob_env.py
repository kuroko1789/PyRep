...
from pyrep.robots.mobiles.omnirob import OmniRob
...
SCENE_FILE = join(dirname(abspath(__file__)), 'scene_omnirob_easy_navigation.ttt')
...
class OmnirobNavigationEnv(gym.Env):
	...
	def _get_state(self):
		sensor_data = self.agent.get_sensor_data() # len(sensor_data) 1368
		target_pos = self.target.get_position(relative_to=self.agent)[:2]
		velocity = self.agent.get_velocity() 
		return np.array(sensor_data + target_pos + velocity) # shape (1373, )
